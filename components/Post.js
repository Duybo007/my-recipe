...
import { addDoc, collection, orderBy, query, onSnapshot, serverTimestamp, setDoc, doc, deleteDoc } from 'firebase/firestore'
...
function Post({ id, username, userImg, img, caption}) {
  ...
  useEffect(()=>
    setHasLiked(
      likes.findIndex((like) => like.id === session?.user?.uid) != -1)    //check if like id is the same as session user id
  ,[likes])
  ...
  return (
    <div className='bg-white my-7 border rounded-sm'>
      ...
      <div className='flex items-center p-5'>
        <img src={userImg} className="rounded-full w-12 h-12 object-contain border p-1 mr-3" />
